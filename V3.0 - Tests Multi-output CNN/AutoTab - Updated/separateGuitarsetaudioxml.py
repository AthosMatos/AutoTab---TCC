...
from Helpers.utils.loadFiles import getFilesPATHS, findFilePath
from Helpers.utils.notes import genNotes
import librosa
import soundfile as sf
import os

DS_NAME = "IDMT-SMT-GUITAR_V2"
OLD_DS_PATH = ds_path + f"\\{DS_NAME}"
FILESXML = getFilesPATHS(OLD_DS_PATH, extension=".xml")
LABELS = genNotes()[0]
NEW_DS_PATH = os.path.dirname(__file__) + "\\new-dataset"

NOTE_SAVE_PATH = f"{NEW_DS_PATH}\\notes"
if not os.path.exists(NOTE_SAVE_PATH):
    os.makedirs(NOTE_SAVE_PATH)

CHORD_SAVE_PATH = f"{NEW_DS_PATH}\\chords"
if not os.path.exists(CHORD_SAVE_PATH):
    os.makedirs(CHORD_SAVE_PATH)


def save_note():
    SAVEPATH = f"{NOTE_SAVE_PATH}\\{curr_note}"  #
...
def save_chord():
    ...
    SAVEPATH = f"{CHORD_SAVE_PATH}\\{SUBFOLDERS}{notes}"  #
    if not os.path.exists(SAVEPATH):
        os.makedirs(SAVEPATH)
...
def prepareSubFoldersString():
    ...
    subF = WAVFILEPATH.split(DS_NAME + "\\")[-1].split("\\")
    filename = subF[-1]
    subF = subF[:-2]
    sub = ""
...
for FILEXML in FILESXML:
    ...
    XMLPATH = FILEXML.split("\\annotation\\")[0]
    WAVFILENAME = FILEXML.split("\\")[-1].split(".")[0] + ".wav"
    ...
    with open(FILEXML, "r") as f:
        ...
        for i in range(0, len(AUDIO_EVENTS)):
            ...
            if next_start_seconds - curr_start_seconds > 0.05:
                notes_for_chord = save(notes_for_chord)
