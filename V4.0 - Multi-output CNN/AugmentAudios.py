...
import os
import librosa
from utils.notes import shiftNote, getHighestLowestNote, genNotes_v2, isNoteHigher
from multiprocessing import cpu_count, Pool, Manager
import numpy as np

WAVFILES_PATHS = getFilesPATHS(
    CUSTOM_DATASETS.path, ignores=["chords"], extension=".wav"
)
...
def genNewShiftedWavs(
    ...
    def isInShiftGones(shift, orEqualZero=False):
        if orEqualZero and shift == 0:
            return True
        for shift_gone in shift_gones:
            if shift_gone == shift:
                return True
        return False
    ...
    while shift <= shiftEnd:
        ...
        if isInShiftGones(shift, orEqualZero=True):
            continue
