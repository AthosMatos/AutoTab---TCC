...
import numpy as np
from audioUtils4 import load, Prepare
import os
import librosa
from consts import (
    notes_class,
...
model = load_model("NotesNeuralNet_STFT.h5")
...
while windowEnd <= maxSecnds:
    audio, sr = load(
        os.path.dirname(__file__) + "/dataset/musics/beach house - clean.wav",
        (windowStart, windowEnd),
    )
    o_env = librosa.onset.onset_strength(y=audio, sr=sr)
    onset_frames = librosa.onset.onset_detect(onset_envelope=o_env, sr=sr)

    spec = Prepare(audio, sr)
