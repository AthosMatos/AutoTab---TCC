from numpy import array, argmax, zeros, random, sin, pi, arange
...
def gen_dataset(n_in, n_out, n_features, n_out_gen, n_samples):
    ...
    for _ in range(n_samples):
        ...
        # generate 2sec sine wave of 16kHz with range -1 to 1
        wave = sin(2 * pi * 440 * arange(32000) / 16000)
        X_WAVE.append(wave)
    ...
    return (X_TIMES, PREVS_TIMES, y_TIMES), (X_NOTES, PREVS_NOTES, y_NOTES), X_WAVE
...
def define_models(n_input_output, n_steps_in, n_features, n_units):
    ...
    encoder_inputs_wave = Input(shape=(samples, 1), name="encoder_inputs_wave")

    conv = Conv1D(32, 3, activation="relu", padding="same", name="encoder_conv2d_1")(
        encoder_inputs_wave
    )
    maxPool = MaxPooling1D(pool_size=2, name="encoder_maxpool_1")(conv)

    conv = Conv1D(64, 3, activation="relu", padding="same", name="encoder_conv2d_2")(
        maxPool
    )
    maxPool = MaxPooling1D(pool_size=2, name="encoder_maxpool_2")(conv)
    flat = Flatten(name="encoder_flatten")(maxPool)

    wave_out = Dense(128, activation="relu", name="wave_out")(flat)
    ...
    state_h_encoder = concatenate(
        [state_h_encoder_f, state_h_encoder_b, wave_out], axis=-1
    )
    state_c_encoder = concatenate(
        [state_c_encoder_f, state_c_encoder_b, wave_out], axis=-1
    )
    ...
    encoder_model = Model(
        [encoder_inputs_wave, encoder_inputs_times, encoder_inputs_notes],
        encoder_states,
        name="encoder",
    )
...
def predict_sequence(infenc, infdec, source, n_steps):
    ...
    # encode
    states = infenc.predict([X_WAVE, X_TIMES, X_NOTES], verbose=0)
...
TIMES, NOTES, WAVES = gen_dataset(n_steps_in, n_steps_out, n_classes, n_out_gen, 10000)
...
# train model
train_model.fit(
    [WAVES, X_TIMES, X_NOTES, PREVS_TIMES, PREVS_NOTES],
    [y_TIMES, y_NOTES],
    epochs=10,
)
...
for _ in range(10):
    TIMES, NOTES, X_WAVE = gen_dataset(n_steps_in, n_steps_out, n_classes, n_out_gen, 1)

    X_TIMES, PREVS_TIMES, y_TIMES = TIMES
    X_NOTES, PREVS_NOTES, y_NOTES = NOTES
    predict = predict_sequence(
        encoder, decoder, [X_TIMES, X_NOTES, X_WAVE], n_steps_out
    )
